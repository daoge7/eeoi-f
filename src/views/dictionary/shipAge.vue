<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('dictionary.shipAge.ageSmall')" width="auto" align="center" show-overflow-tooltip>
        <template slot-scope="{row}">
          <span>{{ row.ageSmall }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('dictionary.shipAge.ageBig')" width="auto" align="center" show-overflow-tooltip>
        <template slot-scope="{row}">
          <span>{{ row.ageBig }}</span>
        </template>
      </el-table-column>
      <el-table-column
    ...
    >
      ...
      >
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('dictionary.shipAge.ageSmall')">
              <el-input v-model="temp.ageSmall" :readonly="dialogStatus=='detail'?true:false" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="$t('dictionary.shipAge.ageBig')">
              <el-input v-model="temp.ageBig" :readonly="dialogStatus=='detail'?true:false" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  methods: {
    getList() {
      ...
      var url = '/shipAge/findShipAgeList'
    ...
    handleUpdate(row) {
      var url = '/shipAge/findShipAgeById/' + row.id
    ...
    handleDelete(row, status) {
      if (status === 'delete') {
        if (confirm('确定要删除？') === true) {
          getQueryInfo('/shipAge/deleteShipAgeById/' + row.id).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          var url = '/shipAge/saveOrUpdateShipAge'
