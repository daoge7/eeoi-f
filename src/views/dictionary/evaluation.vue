<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('dictionary.evaluation.paramname')" width="auto" align="center" show-overflow-tooltip>
        <template slot-scope="{row}">
          <span>{{ row.paramname }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('dictionary.evaluation.paramvalue')" width="auto" align="center" show-overflow-tooltip>
        <template slot-scope="{row}">
    ...
    >
      ...
      >
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('dictionary.evaluation.paramname')">
              <el-input v-model="temp.paramname" :readonly="dialogStatus=='detail'?true:false" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="$t('dictionary.evaluation.paramvalue')">
              <el-input v-model="temp.paramvalue" :readonly="dialogStatus=='detail'?true:false" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    getList() {
      ...
      var url = '/evaluation/findEvaluationList'
    ...
    handleUpdate(row) {
      var url = '/evaluation/findEvaluationById/' + row.id
    ...
    handleDelete(row, status) {
      if (status === 'delete') {
        if (confirm('确定要删除？') === true) {
          getQueryInfo('/evaluation/deleteEvaluationById/' + row.id).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          var url = '/evaluation/saveOrUpdateEvaluation'
