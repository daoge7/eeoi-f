  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <div style="display:flex;align-items: center;">
        ...
        <el-select v-model="listQuery.startYear" :placeholder="$t('common.selectRemind')" class="filter-item" style="width: 130px">
          <el-option v-for="item in yearOptions" :key="item" :label="item" :value="item" />
        </el-select>
        </div>
        <div style="padding:10px 10px 0; display:flex;align-items: center;">
        <label class="radio-label">{{ $t('Analysis.endYear') }}</label>
        <el-select v-model="listQuery.endYear" :placeholder="$t('common.selectRemind')" class="filter-item" style="width: 130px">
          <el-option v-for="item in yearOptions" :key="item" :label="item" :value="item" />
        </el-select>
        </div>
        <div style="padding:10px 10px 0; display:flex;align-items: center;">
        <label class="radio-label">{{ $t('Analysis.rollingCycle') }}</label>
        <el-select v-model="listQuery.rollingCycle" :placeholder="$t('common.selectRemind')" clearable class="filter-item" style="width: 130px">
          <el-option v-for="item in rollingCycleOptions" :key="item.key" :label="item.display_name" :value="item.key" />
        </el-select>
        </div>
        <div style="padding:10px 10px 0; display:flex;align-items: center;">
        <label class="radio-label">{{ $t('Analysis.rollingCycleUnit') }}</label>
        <el-select v-model="listQuery.rollingCycleUnit" :placeholder="$t('common.selectRemind')" clearable class="filter-item" style="width: 130px">
          <el-option v-for="item in rollingCycleUnitOptions" :disabled="item.disabled" :key="item.key" :label="item.display_name" :value="item.key" />
        </el-select>
        </div>
      </div>
...
let rollingCycleUnitOptions = [
  ...
  { key: '航次', display_name: '航次',disabled: true }
]

const rollingCycleOptions = [
...
export default {
  ...
  methods: {
    ...
    adds(val) {
      ...
      this.rollingCycleUnitOptions.forEach(ele => {
        if(ele.key ==='航次') {
           ele.disabled = false
        }
      })
    ...
    handleReset() {
      ...
      this.listQuery.rollingCycle = undefined
