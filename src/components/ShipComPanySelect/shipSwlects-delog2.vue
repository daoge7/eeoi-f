<template>
  <div class="app-container">
    <el-dialog title="船舶列表" width="80%" :append-to-body="true" :visible.sync="add" :before-close="addDetale">
...
<script>
    ...
    export default {
     ...
     created(){
        ...
        methods: {
            addDetale() {
              this.$emit('adds2', false)
            },
          saveRelation() {
              let obj = {}
              if(Array.isArray(this.tableChecked)){
              obj = {
              sptype:this.tableChecked[0].sptype,
              name: this.tableChecked[0].spname + this.tableChecked[0].spnameCn,
              imo: this.tableChecked[0].imono,
              id: this.tableChecked[0].id
              }
          }else {
              obj = {
              name: this.tableChecked.spname + this.tableChecked.spnameCn,
              imo: this.tableChecked.imono,
              sptype:this.tableChecked.sptype,
              id: this.tableChecked.id
              }
          }
          this.$emit('adds2', obj,this.tableChecked[0])
          },
